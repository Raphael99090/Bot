...
const OWNERS       = (process.env.OWNER_ID || "SEU_ID_AQUI").split(",").map(id => id.trim());
const GROQ_API_KEY = process.env.GROQ_API_KEY || "SUA_GROQ_API_KEY_AQUI";
const DATABASE_URL = process.env.DATABASE_URL || null;
const GIPHY_KEY    = process.env.GIPHY_KEY    || "SUA_GIPHY_API_KEY_AQUI";
// ──────────────────────────────────────────────────────────
...
async function fetchGif(query) {
  try {
    ...
    const url    = `https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_KEY}&q=${encodeURIComponent(query)}&limit=1&offset=${offset}&rating=pg-13&lang=en`;
    const res    = await fetch(url);
    const data   = await res.json();
    return data.data?.[0]?.images?.original?.url || null;
  } catch (e) {
    log("warn", `Giphy erro: ${e.message}`);
    return null;
  }
}
// ──────────────────────────────────────────────────────────
...
<body>
  ...
  if (commandName === "giveaway") {
    if (!OWNERS.includes(interaction.user.id)) return interaction.reply({ content: "❌ Só o dono.", ephemeral: true });
  ...
  if (commandName === "gado") {
    ...
    const gif  = await fetchGif("cow moo funny") || "https://media.tenor.com/X6mFCCiJfokAAAAM/cow-moo.gif";
    const embed = new EmbedBuilder().setColor(0x8B4513).setTitle("🐄 Gadômetro").setDescription(`**${alvo.username}** é **${pct}% gado**\n\`[${barra}]\` ${pct}%\n\n${nivel}`).setImage(gif).setFooter({ text:"Resultados científicos 🔬" });
    return interaction.reply({ embeds: [embed] });
  }
  ...
  if (commandName === "beijar") {
    ...
    const gif  = await fetchGif("anime kiss") || "https://media.tenor.com/s-hc_4dBaHkAAAAM/anime-kiss.gif";
    const embed = new EmbedBuilder().setColor(0xff69b4).setTitle("💋 Beijo!").setDescription(`**${interaction.user.username}** beijou **${alvo.username}**! 💕`).setImage(gif);
    return interaction.reply({ embeds: [embed] });
  }
  if (commandName === "abracar") {
    const alvo = interaction.options.getUser("usuario");
    const gif  = await fetchGif("anime hug") || "https://media.tenor.com/od_6o9LBHN8AAAAM/anime-hug.gif";
    const embed = new EmbedBuilder().setColor(0xffa500).setTitle("🤗 Abraço!").setDescription(`**${interaction.user.username}** abraçou **${alvo.username}**! 💛`).setImage(gif);
    return interaction.reply({ embeds: [embed] });
  }
  if (commandName === "tapa") {
    const alvo = interaction.options.getUser("usuario");
    if (alvo.id === interaction.user.id) return interaction.reply({ content: "❌ Auto-tapa? Não.", ephemeral: true });
    const gif  = await fetchGif("anime slap") || "https://media.tenor.com/fhDSYkSbhroAAAAM/anime-slap.gif";
    const embed = new EmbedBuilder().setColor(0xff4500).setTitle("👋 TAPA!").setDescription(`**${interaction.user.username}** deu um tapa em **${alvo.username}**! 😤`).setImage(gif);
    return interaction.reply({ embeds: [embed] });
  }
