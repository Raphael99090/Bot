...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "perfil") {
    ...
    const progresso   = Math.min(Math.floor((u.pontos/proxLvl)*10),10);
    const barra       = "█".repeat(progresso)+"░".repeat(10-progresso);
    const medalha     = pos===1?"🥇":pos===2?"🥈":pos===3?"🥉":`#${pos}`;
    const ultimoDiario= u.ultimo_diario ? `<t:${Math.floor(u.ultimo_diario/1000)}:R>` : "Nunca";
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: "📊 Geral", value: `💰 **Pontos:** ${u.pontos}\n🏆 **Ranking:** ${medalha} de ${total}\n⭐ **Nível:** ${nivel}\n\`[${barra}]\` ${u.pontos}/${proxLvl} pts`, inline: false },
        { name: "⚔️ Duelos", value: `✅ ${u.duelos_ganhos} | ❌ ${u.duelos_perdidos}\n📈 Win rate: **${u.duelos_ganhos+u.duelos_perdidos>0?Math.round(u.duelos_ganhos/(u.duelos_ganhos+u.duelos_perdidos)*100):0}%**`, inline: true },
        { name: "🎰 Cassino", value: `🎰 Slots: **${u.slots_jogados}**\n🃏 BJ: **${u.bj_jogados}**\n🧠 Trivias: **${u.trivia_acertos}**`, inline: true },
        { name: "📅 Diário", value: `Último: ${ultimoDiario}`, inline: false },
        { name: "📜 Missão", value: missaoR ? (missaoR.concluida ? "✅ Concluída" : `⏳ ${missaoR.descricao}`) : "Nenhuma missão activa", inline: false },
      )
      .setFooter({ text: `ID: ${alvo.id}` }).setTimestamp();
    return interaction.editReply({ embeds: [embed] });
  }
