  ...
  if (commandName === "perfil") {
    ...
    try {
      ...
      const missaoTxt = missaoR && !missaoR.concluida
        ? `📜 ${missaoR.descricao.slice(0, 60)}${missaoR.descricao.length > 60 ? "…" : ""}`
        ...
        <style>
          * { margin:0; padding:0; box-sizing:border-box; }
          ...
          .avatar {
            ...
            border:3px solid #5865f2; object-fit:cover;
          }
          .info { position:absolute; top:50px; left:230px; right:30px; }
          .nome { font-size:28px; font-weight:700; }
          .sub  { font-size:16px; color:#a0a0b0; margin-top:6px; }
          .pts  { font-size:20px; font-weight:700; color:#f1c40f; margin-top:10px; }
          .barWrap { width:540px; height:16px; background:#2a2a3e; border-radius:8px; margin-top:14px; overflow:hidden; }
          .barFill { height:100%; background:linear-gradient(90deg,#5865f2,#9b59b6); width:${pctNum}%; }
          .barLabel { font-size:13px; color:#a0a0b0; margin-top:6px; }
          .divider { position:absolute; top:248px; left:230px; right:30px; border-top:1px solid #2a2a3e; }
          .stats { position:absolute; top:262px; left:230px; right:30px; display:flex; justify-content:space-between; }
          .stat { text-align:center; flex:1; }
          .statLabel { font-size:12px; color:#a0a0b0; }
          .statValue { font-size:16px; font-weight:700; margin-top:4px; }
          .footer { position:absolute; bottom:14px; left:230px; right:30px; display:flex; justify-content:space-between; font-size:12px; color:#a0a0b0; }
        </style>
      </head>
      <body>
        <img class="avatar" src="${avatarURL}" />
        <div class="info">
          <div class="nome">${alvo.username.slice(0, 22)}</div>
          <div class="sub">${medalha} de ${total} • Nível ${nivel}</div>
          <div class="pts">${u.pontos.toLocaleString("pt-BR")} pts</div>
          <div class="barWrap"><div class="barFill"></div></div>
          <div class="barLabel">${u.pontos} / ${proxLvl} pts para nível ${nivel + 1}</div>
        </div>
        <div class="divider"></div>
        <div class="stats">
          <div class="stat"><div class="statLabel">⚔️ Duelos</div><div class="statValue">${u.duelos_ganhos}W / ${u.duelos_perdidos}L</div></div>
          <div class="stat"><div class="statLabel">📈 Win Rate</div><div class="statValue">${wr}%</div></div>
          <div class="stat"><div class="statLabel">🎰 Slots</div><div class="statValue">${u.slots_jogados}</div></div>
          <div class="stat"><div class="statLabel">🃏 BJ</div><div class="statValue">${u.bj_jogados}</div></div>
          <div class="stat"><div class="statLabel">🧠 Trivias</div><div class="statValue">${u.trivia_acertos}</div></div>
        </div>
        <div class="footer">
          <span>${missaoTxt}</span>
          <span>ID: ${alvo.id}</span>
        </div>
      </body>
      </html>`;

      const buffer     = await renderHTML(html, 800, 320);
